...
class CataloguePage(BasePage):
    """Class CataloguePage"""
    colors_dict = {'Бежевый': 3, 'Белый': 1, 'Голубой': 4, 'Желтый': 5, 'Зеленый': 6, 'Коричневый': 8, 'Кофейный': 9,
                   'Красный': 13, 'Металлик': 16, 'Молочный': 7, 'Оливковый': 14, 'Оранжевый': 12, 'Принт': 15,
                   'Розовый': 11, 'Серый': 10, 'Синий': 17, 'Фиолетовый': 18, 'Черный': 2}
    ...
    def get_all_items_title(self):
        try:
            return WebDriverWait(self.driver, 30).until(visibility_of_all_elements_located
    ...
    def get_all_items_price(self):
        try:
            return WebDriverWait(self.driver, 10).until(visibility_of_all_elements_located
    ...
    def get_filter_razmer_odezhdy_lst(self):
        ...
        vse_razmery = WebDriverWait(self.driver, 10).until(visibility_of_all_elements_located
    ...
    def get_filter_tzveta_lst(self):
        ...
        vse_tzveta = WebDriverWait(self.driver, 10).until(visibility_of_all_elements_located
                                                          ((By.XPATH, self.all_tzvet)))
        tzveta_lst = [s.text.strip() for s in vse_tzveta]
        return tzveta_lst
    ...
    def select_random_razmer_odezhdy(self):
        ...
        self.wait_url_changed(old_url)
        self.assert_razmer_odezhdy_changed(value)
    ...
    def select_random_tzveta(self):
        ...
        self.wait_url_changed(old_url)
        self.assert_tzvet_changed(value)
    ...
    def fill_in_min_max_price(self):
        ...
        print(f"'От': {min_price} selected")
        print(f"'До': {max_price} selected")
        self.click_primenit_button()
        self.wait_url_changed(old_url)
        self.assert_price_changed(min_price, max_price)
    ...
    def apply_filters_until_items_found(self, reset_method, select_method, max_tries=5):
        ...
        raise AssertionError("No items found")
    ...
    def assert_razmer_odezhdy_changed(self, size_value):
        ...
        if size_value == 'One size':
            assert f"size=One%20size" in current_url, print(f"Filter {size_value} isn't applied, current url: "
                                                              f"{current_url}")
            print(f'Filter: {size_value} applied, current url: {current_url}')
        else:
            assert f'size={size_value}' in current_url, print(f"Filter {size_value} isn't applied, current url: "
                                                              f"{current_url}")
            print(f'Filter: {size_value} applied, current url: {current_url}')

    def assert_tzvet_changed(self, color_value):
        current_url = self.driver.current_url
        assert f"color={str(self.colors_dict[color_value])}" in current_url, print(f"Filter {color_value} isn't applied, current"
                                                                           f" url: {current_url}")
        print(f'Filter: {color_value} applied, current url: {current_url}')

    def assert_price_changed(self, min_price, max_price):
        current_url = self.driver.current_url
        assert f"price={min_price};{max_price} in current url: {current_url}", print(f"Filter 'Цена от {min_price} до"
                                                                                     f" {max_price}' isn't applied, "
                                                                                     f"current url: {current_url}")
        print(f"Filter'Цена от {min_price} до {max_price}' applied, current url: {current_url}")


    # Methods

    def check_page_parameters(self):
        self.assert_page_url("https://12storeez.com/catalog/odezda/womencollection")
        self.assert_page_title(locator=self.page_title, exp_title='Одежда')

    def apply_filters(self):
        self.open_filter_razmer_odezhdy()
        self.select_random_razmer_odezhdy()
        self.apply_filters_until_items_found(max_tries=5, reset_method=self.reset_razmer_odezhdy_filter,
                                             select_method=self.select_random_razmer_odezhdy)
        self.open_filter_tzveta()
        self.select_random_tzveta()
        self.apply_filters_until_items_found(max_tries=5, reset_method=self.reset_tzvet_filter,
                                             select_method=self.select_random_tzveta)
        self.dynamic_scroll_wait()
        self.open_price_filter()
        self.fill_in_min_max_price()
        self.dynamic_scroll_wait()
        self.apply_filters_until_items_found(max_tries=5, reset_method=self.reset_tzena_filter,
                                             select_method=self.fill_in_min_max_price())
        self.dynamic_scroll_wait()
